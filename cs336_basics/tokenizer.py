...
class Tokenizer:
    def __init__(
        ...
        self.special_split_pattern = (
            "(" + "|".join(map(re.escape, self.special_tokens)) + ")"
        )
        self.special_split_re = re.compile(self.special_split_pattern)
        self.word_re = re.compile(self.pattern)
    ...
    def encode(self, text: str) -> list[int]:
        ...
        if self.special_tokens:
            samples = self.special_split_re.split(text)
        else:
            samples = [text]
        ...
        for sample in samples:
            if sample in self.special_tokens_set:
                ...
                sample = self.word_re.finditer(sample)
                for word in sample:
    ...
    def encode_iterable(self, iterable: Iterable[str]) -> Iterable[int]:
        for text in iterable:
            ...
            for sample in samples:
                if sample in self.special_tokens_set:
                    yield self.vocab_inv[sample.encode("utf-8")]
                else:
                    sample = self.word_re.finditer(sample)
                    for word in sample:
                        word = [bytes([x]) for x in word.group().encode("utf-8")]
